...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Hard navigation to bust layout cache and re-fetch auth state
      ...
      <div className="w-full max-w-md">
        <div className="bg-[rgb(var(--surface))] rounded-lg border border-hairline p-8">
          ...
          <div className="mt-6 text-center space-y-2">
            <Link
              href="/reset-password"
              className="block font-sans text-sm text-[rgb(var(--text-tertiary))] hover:text-[rgb(var(--text-primary))]"
            >
              Forgot password?
            </Link>
            <p className="font-sans text-sm text-[rgb(var(--text-tertiary))]">
              Don&apos;t have an account?{' '}
              <Link href="/signup" className="text-gold-ink hover:underline">
                Sign up
              </Link>
            </p>
          </div>
