...
export default function GenesisReadingPage({ params }: Props) {
  const { slug } = params;
  ...
  if (slug.length === 1) {
    ...
    if (!isNaN(chapterNum) && chapterNum >= 1 && chapterNum <= 50) {
      ...
      if (book) {
        redirect(`/ot/genesis/${book.id}/${chapterNum}`);
      }
    }
  ...
  if (slug.length === 2) {
    ...
    if (isNaN(chapterNum)) {
      redirect(`/ot/genesis/${bookId}/${book.chapters[0]}`);
    }
    ...
    if (!book.chapters.includes(chapterNum)) {
      ...
      if (correctBook) {
        redirect(`/ot/genesis/${correctBook.id}/${chapterNum}`);
      }
      redirect('/');
    }
