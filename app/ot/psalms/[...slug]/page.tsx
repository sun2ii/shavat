...
export default function PsalmsReadingPage({ params }: Props) {
  const { slug } = params;
  ...
  if (slug.length === 1) {
    ...
    if (!isNaN(psalmNum) && psalmNum >= 1 && psalmNum <= 150) {
      ...
      if (collection) {
        redirect(`/ot/psalms/${collection.id}/${psalmNum}`);
      }
    }
    ...
    if (collection) {
      redirect(`/ot/psalms/${collection.id}/${collection.psalms[0]}`);
    }
  ...
  if (slug.length === 2) {
    ...
    if (isNaN(psalmNum)) {
      redirect(`/ot/psalms/${collectionId}/${collection.psalms[0]}`);
    }
    ...
    if (!collection.psalms.includes(psalmNum)) {
      ...
      if (correctCollection) {
        redirect(`/ot/psalms/${correctCollection.id}/${psalmNum}`);
      }
      redirect('/');
    }
