  ...
  return (
    <div className="font-inter min-h-screen flex flex-col" style={{ background: '#F7F5F1', color: '#1F2E24' }}>
      ...
      <section className="relative h-[240px] sm:h-[280px] md:h-[340px]">
        ...
        <header className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-6 p-4 sm:px-8 sm:pt-5 md:px-11">
          <nav className="flex flex-wrap items-center gap-3 sm:gap-5 md:gap-7 text-[10px] sm:text-[11px] md:text-[12.5px] tracking-[2px] text-white/85 font-medium">
            ...
            <Link href="/pricing" className="text-[#FDFCF9] font-bold border-b-2 border-[#C8A248] pb-1">PRICING</Link>
        ...
        <div className="relative z-10 px-6 sm:px-10 md:px-[70px] pt-6 sm:pt-8 md:pt-10 max-w-[500px]">
          <h1 className="font-playfair text-3xl sm:text-4xl md:text-[56px] font-medium text-[#FDFCF9] m-0" style={{ textShadow: '0 2px 20px rgba(12,18,10,.5)' }}>Pricing</h1>
          <p className="text-sm sm:text-base md:text-[17px] leading-relaxed text-[#FDFCF9] mt-3 sm:mt-4 max-w-[380px]" style={{ textShadow: '0 1px 14px rgba(12,18,10,.55)' }}>
      ...
      <section className="flex flex-col sm:flex-row justify-center gap-5 sm:gap-6 md:gap-7 px-4 sm:px-8 md:px-12 py-6 sm:py-6 items-center sm:items-start flex-1">
        {/* FREE PLAN */}
        <div className="w-full max-w-[300px] sm:w-[260px] md:w-[280px] bg-[#F9F6EF] border border-[#E0DACB] rounded-xl p-5 sm:p-6 text-center">
          <span className="w-11 h-11 sm:w-12 sm:h-12 rounded-full bg-[#3A4A2E] inline-flex items-center justify-center">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#C8A248" strokeWidth="1.3" style={svgDebugStyle}>
          ...
          <div className="text-[11px] sm:text-xs text-[#4A4A45] mt-1">Start reading with clarity.</div>
          <div className="grid grid-cols-2 gap-1.5 sm:gap-2 mt-4 text-left">
            {FREE_FEATURES.map((feature) => (
              <div key={feature} className="flex items-center gap-1.5 text-[10px] sm:text-[11px] text-[#2E3A30]">
                <CheckIcon />
                {feature}
              </div>
            ))}
          </div>
          <a href="#" className="block bg-[#FDFCF9] border border-[#C9C2A8] text-[#2E3A30] py-2.5 rounded mt-4 text-[10px] sm:text-[11px] tracking-[2px] font-semibold" style={linkDebugStyle}>
            START FREE
          </a>
        </div>
        ...
        <div className="w-full max-w-[320px] sm:w-[280px] md:w-[300px]">
          <div className="bg-[#3A4A2E] rounded-t-lg py-1.5 flex justify-center">
            <span className="bg-[#C8A248] text-[#1F2E24] text-[9px] sm:text-[10px] tracking-[2px] font-semibold px-4 sm:px-5 py-1">MOST POPULAR</span>
          </div>
          <div className="bg-[#F9F6EF] border border-[#E0DACB] border-t-0 rounded-b-xl p-5 sm:p-6 text-center">
            <span className="w-11 h-11 sm:w-12 sm:h-12 rounded-full bg-[#3A4A2E] inline-flex items-center justify-center">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#C8A248" strokeWidth="1.3" style={svgDebugStyle}>
            ...
            <div className="mt-2">
              ...
              <span className="font-playfair text-xs sm:text-sm text-[#4A4A45]"> / month</span>
            </div>
            <div className="text-[10px] sm:text-[11px] text-[#4A4A45] mt-0.5">or $99 / year</div>
            <div className="text-[11px] sm:text-xs text-[#4A4A45] mt-1.5">Go deeper with guided reflection.</div>
            ...
            <a href="#" className="block bg-[#3A4A2E] text-[#F7F5F1] py-2.5 rounded mt-4 text-[10px] sm:text-[11px] tracking-[2px] font-semibold" style={linkDebugStyle}>
              CHOOSE PREMIUM
            </a>
          </div>
        </div>
      ...
      <section className="px-4 sm:px-8 md:px-12 pb-6 sm:pb-7">
        <div className="bg-[#EDE9DC] border border-[#E0DACB] rounded-lg flex flex-col sm:flex-row items-center gap-4 sm:gap-5 md:gap-7 p-4 sm:p-5 md:px-8">
          <svg width="40" height="34" viewBox="0 0 150 120" fill="none" stroke="#C8A248" strokeWidth="1.2" className="flex-none hidden sm:block" style={svgDebugStyle}>
          ...
          <div className="flex-1 min-w-0 text-center sm:text-left">
            <div className="font-playfair text-lg sm:text-xl md:text-[26px] font-medium text-[#1F2E24]">Start with clarity. Grow with intention.</div>
            <div className="text-xs sm:text-[13px] text-[#4A4A45] mt-1">Join thousands engaging Scripture with purpose and peace.</div>
          </div>
          <Link href="#" className="flex-none inline-flex items-center gap-2.5 bg-[#3A4A2E] text-[#F7F5F1] px-5 sm:px-6 py-2.5 sm:py-3 rounded-md text-[10px] sm:text-[11px] tracking-[2px] font-semibold" style={linkDebugStyle}>
            <svg width="12" height="12" viewBox="0 0 40 40" fill="none" stroke="#C8A248" strokeWidth="2" style={svgDebugStyle}>
