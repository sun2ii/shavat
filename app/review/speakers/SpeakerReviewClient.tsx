...
export default function SpeakerReviewClient() {
  ...
  const [commentMode, setCommentMode] = useState<'rejected' | 'unsure' | null>(null);
  ...
  const saveReview = useCallback(
    async (status: ReviewStatus, proposedSegments?: Segment[], comment?: string) => {
  ...
  const handleNo = useCallback(() => setCommentMode('rejected'), []);
  const handleNotSure = useCallback(() => setCommentMode('unsure'), []);
  const handleSubmitComment = useCallback(() => {
    if (commentMode) {
      saveReview(commentMode, undefined, commentText || undefined);
    }
  }, [commentMode, commentText, saveReview]);
  const handleSkipComment = useCallback(() => {
    if (commentMode) {
      saveReview(commentMode);
    }
  }, [commentMode, saveReview]);
        ...
        <div className="max-w-4xl mx-auto px-6 py-3">
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              <Link href="/review" className="font-sans text-xs text-gold-ink hover:underline">
                ← Back
              </Link>
              {viewMode === 'expert' && (
                <select
          ...
          {/* Expert mode: simplified stats and filters */}
        ...
        {filteredVerses.length === 0 ? (
          ...
          viewMode === 'simple' ? (
            <SimpleReviewCard
              ...
              onCancelComment={() => { setCommentMode(null); setCommentText(''); }}
            />
          ) : (
            <ExpertReviewCard
              verse={currentVerse}
...
interface SimpleReviewCardProps {
  ...
  canGoPrev: boolean;
  canGoNext: boolean;
  commentMode: 'rejected' | 'unsure' | null;
  commentText: string;
...
function SimpleReviewCard({
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      {/* Question or Comment Input */}
      {commentMode ? (
        <div className="w-full max-w-md mb-8">
          <p className="font-sans text-sm text-muted mb-3 text-center">
            {commentMode === 'rejected' ? "What's wrong?" : "What are you unsure about?"} (optional)
          </p>
          <textarea
            value={commentText}
            onChange={(e) => onCommentChange(e.target.value)}
            placeholder="Add a comment..."
            className="w-full px-4 py-3 bg-[rgb(var(--surface))] border border-hairline rounded-lg font-sans text-sm text-[rgb(var(--text-primary))] resize-none focus:outline-none focus:border-gold"
            rows={2}
            autoFocus
          />
          <div className="flex gap-3 mt-3 justify-center">
            <button
              ...
              className={`font-sans text-sm font-semibold px-6 py-2 text-white rounded-lg transition-colors ${
                commentMode === 'rejected' ? 'bg-red-500 hover:bg-red-600' : 'bg-amber-500 hover:bg-amber-600'
              }`}
            >
              Submit
            </button>
            <button
              ...
              className="font-sans text-sm px-6 py-2 border border-hairline rounded-lg hover:bg-[rgb(var(--bg-secondary))] transition-colors"
            >
              Skip
            </button>
            <button
              onClick={onCancelComment}
              className="font-sans text-sm px-4 py-2 text-faint hover:text-muted transition-colors"
            >
              Cancel
            </button>
          </div>
